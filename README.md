...
>   [!IMPORTANT]
>
>   This package uses [`GLMakie.jl`](https://docs.makie.org/stable/) for visualization of waveguide structures. For three-dimensional plots (`Waveguide.visualize!`) `OpenGL` is necessary as backend, since `Cairo` does not support `volume` plots.
